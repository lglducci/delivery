...
DROP TABLE IF EXISTS config_meios_recebimento CASCADE;
...
---------------------------------------------------
 ...
 -- ✅ VERSÃO FINAL – ESTRUTURA COMPATÍVEL COM O BANCO REAL
...
CREATE TABLE empresas (
    id BIGSERIAL PRIMARY KEY,
    nome TEXT NOT NULL,
    tipo TEXT CHECK (tipo IN ('ME','MEI','LTDA','SA','EIRELI','ASSOCIACAO')),
    documento TEXT,
    criado_em TIMESTAMP DEFAULT now(),
...
ALTER TABLE public.categorias_gerenciais
...
ALTER TABLE public.transacoes
ADD COLUMN IF NOT EXISTS lado_transferencia text;
...
REFERENCES transacoes(id);
  ...
  ALTER TABLE public.transacoes 
ADD COLUMN forma_pagamento   TEXT default 'avista' ;      -- PIX | DINHEIRO | DEBITO | CREDITO | FIADO


ALTER TABLE transacoes
ADD CONSTRAINT chk_transacoes_forma_pagamento
CHECK (
  forma_pagamento IN (
    'avista',
    'pix',
...
ADD COLUMN forma_pagamento TEXT NULL,      -- PIX | DINHEIRO | DEBITO | CREDITO | FIADO

ALTER TABLE public.contas_a_pagar
ADD COLUMN forma_operacao VARCHAR(30);
 
ALTER TABLE public.contas_a_pagar
ADD CONSTRAINT chk_forma_operacao_cp
CHECK (forma_operacao IN (
    'FORNECEDOR',
    'FINANCIAMENTO',
    'CARTAO'
));
