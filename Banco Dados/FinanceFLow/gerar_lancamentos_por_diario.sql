...
BEGIN
  ...
  WHERE id = p_diario_id
    ...
    AND modelo_codigo NOT IN ('PAGAR','RECEBER','COMPRA_CARTAO')
    AND modelo_codigo NOT LIKE 'ESTORNO%';
  ...
  LOOP
    ...
    -- pega código da conta do modelo
    SELECT codigo
    INTO v_conta_codigo
    FROM contab.contas
    ...
    IF v_conta_contabil_id IS NOT NULL
       AND v_conta_codigo LIKE '1.1.1%'  AND COALESCE(r_linha.perna_fixa , true) = true 
