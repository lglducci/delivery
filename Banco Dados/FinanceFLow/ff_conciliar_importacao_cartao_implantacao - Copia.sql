    ...
    LOOP
        ...
        */
         ...
         IF v_compra_id IS NULL THEN
           ...
           /* IF r.data_compra >= v_data_implantacao THEN
                  v_data_compra := r.data_compra;
              ELSE
                  v_data_compra := v_data_implantacao;
              END IF;
          ...
          SELECT public.ff_normalizar_data_compra_implantacao(
                r.data_compra,        -- data histórica original
                v_mes_referencia,     -- mês de referência da fatura
                v_fechamento_dia      -- dia de corte/fechamento
            )
            INTO v_data_compra;
            ...
            SELECT public.ff_registrar_compra_credito(
                p_empresa_id,
                v_cartao_nome,
                '[IMPLANTAÇÃO] ' || r.estabelecimento,
                v_valor_total_implantacao,
                v_parcelas_restantes,

                r.data_compra,        -- data real/histórica da compra
                ...
                v_data_compra         -- início das parcelas, normalizado no período
            )
            INTO v_ultima_transacao_id;
