...
BEGIN
    ...
    IF v_qtd_processadas = 0 THEN
        v_resultado :=
            public.ff_conciliar_importacao_cartao_implantacao(
                p_empresa_id,
                p_importacao_id,
                p_data_referencia
            );
    ELSE
        ...
        ----------------------------------------------------------
        v_resultado :=
            public.ff_conciliar_importacao_cartao_normal(
                p_empresa_id,
                p_importacao_id,
                p_data_referencia
            );
    ...
    --------------------------------------------------------------
    -- PROCESSAMENTO CONTÁBIL AUTOMÁTICO
    --------------------------------------------------------------
    PERFORM contab.ff_processa_automatico(
        p_empresa_id
    );
