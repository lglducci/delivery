...
BEGIN
    ...
    ------------------------------------------------------------------
    -- MESES/COMPETÊNCIAS DO PERÍODO INFORMADO NA PRÓPRIA PROCEDURE
    ------------------------------------------------------------------
    meses_recorrentes_periodo AS (
    ...
    ------------------------------------------------------------------
    -- GRADE DE CONTAS RECORRENTES POR COMPETÊNCIA
    ------------------------------------------------------------------
    recorrentes_periodo AS (
        SELECT
            cr.empresa_id,
            cr.id::BIGINT AS recorrente_id,
            cr.descricao::TEXT AS descricao,
            cr.contabil_id::BIGINT AS contabil_id,
            cr.valor_padrao::NUMERIC AS valor_padrao,
            mrp.competencia,
            (
                mrp.competencia
                + (
                    LEAST(
                        cr.dia_vencimento,
                        EXTRACT(DAY FROM (mrp.competencia + INTERVAL '1 month' - INTERVAL '1 day'))::INTEGER
                    ) - 1
                ) * INTERVAL '1 day'
            )::DATE AS vencimento_previsto
        FROM public.contas_recorrentes cr
        CROSS JOIN meses_recorrentes_periodo mrp
        WHERE cr.empresa_id = p_empresa_id
          AND cr.ativo = TRUE
          AND mrp.competencia >= date_trunc('month', cr.criado_em)::DATE
    ),

    ------------------------------------------------------------------
    -- LINHAS DE CONTAS RECORRENTES
    ------------------------------------------------------------------
    linhas_recorrentes AS (
        SELECT
            rp.empresa_id,
            ('R-' || rp.recorrente_id::TEXT || '-' || TO_CHAR(rp.competencia, 'YYYYMM'))::TEXT AS chave_divida,
            'RECORRENTE'::TEXT AS tipo_divida,
            rp.competencia::DATE AS data_ordenacao,
            1::INTEGER AS ordem,
            0::BIGINT AS parcela_ordenacao,
            NULL::BIGINT AS pagar_id,
            NULL::BIGINT AS pagar_lote_id,
            NULL::BIGINT AS parcela_num,
            NULL::BIGINT AS parcelas,
            CASE WHEN crg.id IS NOT NULL THEN 'PAGAMENTO_RECORRENTE' ELSE 'RECORRENTE_EM_ABERTO' END::TEXT AS etapa,
            NULL::DATE AS data_apropriacao,
            CASE
                WHEN crg.id IS NOT NULL THEN COALESCE(t.data_movimento, crg.data_pagamento)::DATE
                ELSE NULL::DATE
            END AS data_movimento,
            rp.vencimento_previsto::DATE AS vencimento,
            COALESCE(
                md.conta_debito,
                CASE WHEN cc.id IS NOT NULL THEN cc.codigo || ' - ' || cc.nome END
            )::TEXT AS conta_debito,
            md.conta_credito::TEXT AS conta_credito,
            CASE
                WHEN crg.id IS NOT NULL THEN COALESCE(crg.valor_gerado, t.valor, md.valor_total)
                ELSE rp.valor_padrao
            END::NUMERIC AS valor_total,
            COALESCE(NULLIF(TRIM(crg.descricao), ''), rp.descricao)::TEXT AS descricao,
            CASE WHEN crg.id IS NOT NULL THEN 'pago' ELSE 'aberto' END::TEXT AS status_conta,
            md.diario_id::BIGINT AS diario_id
        FROM recorrentes_periodo rp
        LEFT JOIN public.contas_recorrentes_geradas crg
          ON crg.empresa_id = rp.empresa_id
         AND crg.recorrente_id = rp.recorrente_id
         AND date_trunc('month', crg.competencia)::DATE = rp.competencia
        LEFT JOIN public.transacoes t
          ON t.empresa_id = crg.empresa_id
         AND t.id = crg.despesa_id
        LEFT JOIN movimentos_diario md
          ON md.empresa_id = t.empresa_id
         AND md.transacao_id = t.id
        LEFT JOIN contab.contas cc
          ON cc.empresa_id = rp.empresa_id
         AND cc.id = rp.contabil_id
    ...
    ------------------------------------------------------------------
    linhas_faturas_abertas AS (
        SELECT
            f.empresa_id,

            ('F-' || f.id::TEXT)::TEXT AS chave_divida,

            'FATURA_CARTAO'::TEXT AS tipo_divida,

            f.vencimento::DATE AS data_ordenacao,
            ...
            'FATURA_EM_ABERTO'::TEXT AS etapa,

            NULL::DATE AS data_apropriacao,

            NULL::DATE AS data_movimento,

            f.vencimento::DATE AS vencimento,

            NULL::TEXT AS conta_debito,

            NULL::TEXT AS conta_credito,

            COALESCE(f.valor_total, 0)::NUMERIC AS valor_total,

            (
                'Fatura cartão ' ||
                COALESCE(c.nome, 'Cartão')
            )::TEXT AS descricao,

            COALESCE(f.status, 'aberta')::TEXT AS status_conta,
        ...
        WHERE f.empresa_id = p_empresa_id
          ...
          AND UPPER(
                COALESCE(f.status, 'ABERTA')
              ) <> 'PAGA'

          AND f.vencimento::DATE
              BETWEEN p_data_ini AND p_data_fim
    ),
    ...
    resultado AS (
        ...
        SELECT
            3::INTEGER AS bloco_ordenacao,
            lr.empresa_id,
            lr.chave_divida,
            lr.tipo_divida,
            lr.data_ordenacao,
            lr.ordem,
            lr.parcela_ordenacao,
            lr.pagar_id,
            lr.pagar_lote_id,
            lr.parcela_num,
            lr.parcelas,
        ...
        FROM linhas_faturas_abertas lf
    )
