...
BEGIN
    ...
    ------------------------------------------------------- */
  -- SELECT contab.ff_get_or_create_sem_parceiro(p_empresa_id ) AS parceiro_id;
  v_sem_parceiro_cnpj := '00000000000000';
  v_sem_parceiro_cnpj:=5;
    /* -------------------------------------------------------
...
LEFT JOIN public.contas_financeiras cf
  ...
  AND t.data_movimento::date = p_data
  AND NULLIF(trim(t.evento_codigo), '') IS NOT NULL;
