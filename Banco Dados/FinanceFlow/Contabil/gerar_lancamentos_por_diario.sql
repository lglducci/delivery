...
BEGIN
 ...
 IF upper(trim(v_modelo.codigo)) = 'RECEBIMENTO_RECEBER' THEN

    IF r_linha.dc = 'D' THEN
        v_conta_final := COALESCE(
            v_diario.contabil_id,
            v_conta_contabil_id
        );
    ELSE
        v_conta_final := r_linha.conta_id;
...
ELSE
    ...
    IF v_conta_contabil_id IS NOT NULL
       AND v_conta_codigo LIKE '1.1.%'
       AND v_conta_codigo NOT LIKE '1.1.3.%'
       AND r_linha.perna_fixa IS TRUE
       AND r_linha.conta_id <> v_conta_contabil_id
       AND upper(trim(COALESCE(v_modelo.codigo, ''))) NOT IN (
           'RECEBER_CARTAO',
...
IF upper(trim(v_modelo.codigo)) = 'PAGTO_RECEBIVEIS' THEN
