 CREATE OR REPLACE FUNCTION public.fn_importar_extrato(
    ...
    p_lancamentos jsonb,
    p_dados_importacao_pdf jsonb DEFAULT NULL
)
RETURNS jsonb
LANGUAGE plpgsql
AS $$
...
IF p_dados_importacao_pdf IS NOT NULL THEN

    INSERT INTO public.importacao_extrato_pdf (
    ...
    VALUES (
        ...
        p_dados_importacao_pdf->>'banco',

        (p_dados_importacao_pdf->>'data_inicio')::date,

        (p_dados_importacao_pdf->>'data_fim')::date,

        (p_dados_importacao_pdf->>'quantidade')::integer,

        p_dados_importacao_pdf->'diagnostico'
...
WHERE t.empresa_id = c.empresa_id
    ...
    LOOP
       ...
       IF unaccent(upper(v_historico)) LIKE '%SALDO%' THEN
        ...
        ELSIF upper(v_historico) LIKE '%CHEQUE%' THEN
            v_forma := 'avista'; 
        ELSIF unaccent(upper(v_historico)) LIKE '%CARTAO%DEBITO%' THEN
            v_forma := 'cartao_debito';

        ELSIF unaccent(upper(v_historico)) LIKE '%CARTAO%CREDITO%' THEN
            v_forma := 'cartao_credito';

            ELSIF unaccent(upper(v_historico)) LIKE '%CARTAO%'
            OR unaccent(upper(v_historico)) LIKE '%CARD%' THEN
                v_forma := 'cartao_credito';
       ...
       IF upper(v_historico) LIKE '%RENDIMENTO%'
                ...
                ELSIF unaccent(upper(v_historico)) LIKE '%CARTAO%CREDITO%'
                OR unaccent(upper(v_historico)) LIKE '%CARTAO%DEBITO%' THEN
  ...
  v_natureza_movimento := nullif(
    trim(
        coalesce(
            v_item->>'natureza_movimento',
            v_item->>'natureza_financeira',
            ''
        )
    ),
    ''
...
IF v_natureza_movimento IS NOT NULL THEN

    v_tipo_evento := nullif(
        trim(coalesce(v_item->>'tipo_evento', '')),
        ''
    );

    v_classificacao := nullif(
        trim(coalesce(v_item->>'classificacao', '')),
        ''
    );
...
END IF;
     ...
     -- 1) procura por CNPJ/CPF primeiro
        ...
        -- BLOQUEIO SOMENTE SE EXISTIR cascade ; CONFIGURAÇÃO DE MESMA TITULARIDADE
...
IF upper(coalesce(v_natureza_movimento, '')) = 'PAGAMENTO_OPERADORA' THEN
    v_modelo_codigo := 'PAGTO_RECEBIVEIS';
END IF;
        ...
        VALUES (
            ...
            v_lote_conciliacao_id,
            v_conta_id,
            v_natureza_movimento,
            p_lancamentos,
             CASE
                ------------------------------------------------------------------
                -- RECEBÍVEL
                ------------------------------------------------------------------
                WHEN upper(coalesce(v_natureza_movimento, '')) = 'RECEBIVEL'
                    AND v_forma IS NOT NULL
                    AND v_data IS NOT NULL
                ...
                ------------------------------------------------------------------
                -- PAGAMENTO DA OPERADORA
                ------------------------------------------------------------------
                WHEN upper(coalesce(v_natureza_movimento, '')) = 'PAGAMENTO_OPERADORA'
