 CREATE OR REPLACE FUNCTION public.ff_registrar_compra_credito(
    ...
    p_fatura_id BIGINT DEFAULT NULL
)
...
FOR v_parcela IN 1..p_parcelas LOOP
    ...
    --------------------------------------------------------------
    v_data_parcela := ff_calcula_data_parcela(
        v_data_inicio_parcela,
        v_parcela
    );
    ...
    --------------------------------------------------------------
    -- PRIMEIRA parcela:
    -- se a importação já informou a fatura, usa ela diretamente.
    --
    -- Demais parcelas:
    -- deixa a get_or_create descobrir/criar a fatura normalmente.
    --------------------------------------------------------------
    IF v_parcela = 1
       AND p_fatura_id IS NOT NULL
    THEN
