...
DECLARE
    ...
    v_data_compra_historica DATE;
    v_limite_fatura DATE;
    v_periodo_ini  DATE;
...
BEGIN
    ...
    /*
  Último dia do mês da fatura implantada.
  Exemplo: referência 06/2026 => 30/06/2026.
*/
v_limite_fatura :=
    (
        v_mes_referencia
    ...
    --------------------------------------------------------------
      ...
      v_fatura_id := public.ff_get_or_create_fatura(
    p_empresa_id,
    v_cartao_nome,
    v_mes_referencia
);
...
IF v_fatura_id IS NULL THEN
    RAISE EXCEPTION
        'Não foi possível localizar/criar a fatura para referência % e cartão %.',
        p_data_referencia,
        v_cartao_id;
END IF;
    ...
    LOOP
        ...
        */
        ...
        ----------------------------------------------------------
        -- CORRIGE O ANO HISTÓRICO DA COMPRA
        --
        -- Exemplo:
        -- fatura 06/2026
        -- compra parcelada exibida como 09/09
        -- não pode ser 09/09/2026; deve ser 09/09/2025.
        ----------------------------------------------------------
        v_data_compra_historica := r.data_compra;

        IF v_data_compra_historica > v_limite_fatura THEN
            v_data_compra_historica :=
                (
                    v_data_compra_historica
                    - INTERVAL '1 year'
                )::date;
        ...
        IF v_data_compra_historica > CURRENT_DATE THEN
                RAISE EXCEPTION
                    'Data histórica futura inválida: %, estabelecimento %, fatura referência %.',
                    v_data_compra_historica,
                    r.estabelecimento,
                    v_mes_referencia;
            END IF;
        ...
        IF v_compra_id IS NULL THEN
            ...
            SELECT public.ff_registrar_compra_credito(
                ...
                /*
                  Mantém a data histórica no cabeçalho da compra. EU TIREI ISSO E COLOQUEI A DATA NORMALIZADA 
                */
                v_data_compra, 
...
(
    ...
    PERFORM contab.marcar_reprocessamento(p_empresa_id, v_periodo_ini  );
