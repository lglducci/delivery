...
END IF;
    ...
    --------------------------------------------------------------
            -- MENOR DATA EFETIVAMENTE GERADA/CONCILIADA
            -- PARA REPROCESSAMENTO CONTÁBIL
            --------------------------------------------------------------
            SELECT MIN(
                COALESCE(
                    ct.data_compra,
            ...
            INTO v_data_reprocessamento
            FROM public.conciliacao_cartoes cc
            LEFT JOIN public.cartoes_transacoes ct
                ON ct.id = cc.transacao_cartao_id
                AND ct.empresa_id = cc.empresa_id
            WHERE cc.empresa_id = p_empresa_id
            AND cc.importacao_id = p_importacao_id
            AND cc.status_conciliacao = 'conciliado';

            IF v_data_reprocessamento IS NOT NULL THEN
                PERFORM contab.marcar_reprocessamento(
                    p_empresa_id,
                    v_data_reprocessamento
                );
            END IF;
